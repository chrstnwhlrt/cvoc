...
public class TransToPinyinFlashCardTest extends javax.swing.JFrame implements
		Runnable {
	...
	public static void startTest(SqliteHandler sqlite, ArrayList<Vocable> vocs) {
		TransToPinyinFlashCardTest inst = new TransToPinyinFlashCardTest(
				sqlite, vocs);
		SwingUtilities.invokeLater(inst);
	}

	public TransToPinyinFlashCardTest(SqliteHandler sqlite,
			ArrayList<Vocable> vocs) {
		super();
		this.sqlite = sqlite;
	...
	private void initGUI() {
		try {
			...
			}
			{
				...
				getContentPane().add(translationLabel);
				translationLabel.setText("??");
				translationLabel.setBounds(12, 65, 467, 259);
				translationLabel.setFont(CVoc.getFont25plain());
				translationLabel.setHorizontalAlignment(JLabel.CENTER);
				translationLabel.setBorder(BorderFactory
	...
	private boolean displayResult() {
		if (this.topicLabel.isVisible()) {
			...
			this.translationLabel.setText(this.currentVoc.getPinyin() + " ["
	...
	private boolean displayNextVocable() {
		...
		if (this.vocIt.hasNext()) {
			...
			this.translationLabel.setText(this.currentVoc.getTranslation());
